...
8. **A.** Forced tool selection with a specific named tool is exactly the mechanism for guaranteeing a specific call happens before anything else proceeds. Worth being precise beyond the immediate question: the forced choice normally needs to be reset (typically back to `"auto"`) after that first turn completes — leaving it forced would strand the agent, unable to call anything except `get_customer` on every subsequent turn too. (B and C both leave the choice partially or fully to the model, which is the opposite of "guarantee." D is false — `tool_choice` is precisely the mechanism designed for this.)
...
12. **A.** Read-then-Write is the documented, reliable fallback exactly when `Edit`'s unique-match requirement can't be satisfied — this is the exam guide's own stated skill. (A real alternative worth knowing, though not what's being asked here: expanding the edit's anchor text until it's unique again, rather than replacing the whole file, is arguably safer in a workspace with concurrent changes — Read+Write is the fallback when that's impractical, not the only correct move in every case.) (B doesn't address the actual cause of the failure. C swaps a safer, purpose-built tool for a less precise one under a false safety claim. D solves the problem indirectly with an unneeded extra step when Read+Write solves it directly.)
