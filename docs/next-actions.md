...
- [ ] Coachgremlin: author Module 04 (Agentic Loops and Multi-Agent Orchestration), continuing `resolve` — both tiers together, doubt-driven-development before done. This is where the coordinator agent and the real hook enforcing verify-before-refund first exist; Module 03's own `process_refund` defense-in-depth becomes testable as a *second* layer alongside it, not the only layer. `check_module_03` is ready to chain.
- [ ] `backend.py`'s stated deferral (Module 04 owns refund persistence/idempotency, Module 03 only verifies the decision) needs to actually be honored when Module 04 is authored — don't let the stateful backend accidentally re-implement what Module 03 already verifies, and don't let it skip persistence either.
