...
def answer_question_with_governance(
    ...
    """
    # BUG: calls answer_question (which calls model_client internally) FIRST,
    # then checks retrieved content for sensitivity only afterward. The
    # returned GovernedAnswer looks correct (answer nulled out when flagged),
    # but model_client has already seen the sensitive content by the time
    # this function decides to withhold it.
