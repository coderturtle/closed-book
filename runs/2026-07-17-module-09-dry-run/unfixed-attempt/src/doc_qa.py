...
def refresh_index(index: DocIndex, documents: Dict[str, str], chunk_size: int = 12) -> DocIndex:
    ...
    changes, without paying the cost of a full rebuild every time."""
    new_chunks = {doc_id: list(chunks) for doc_id, chunks in index.chunks.items()}
    new_hashes = dict(index.content_hashes)
    for doc_id, text in documents.items():
        current_hash = _content_hash(text)
        if doc_id not in new_chunks or new_hashes.get(doc_id) != current_hash:
            new_chunks[doc_id] = chunk_document(doc_id, text, chunk_size)
            new_hashes[doc_id] = current_hash
