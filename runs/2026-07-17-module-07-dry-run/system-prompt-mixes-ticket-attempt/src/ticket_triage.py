...
def classify_ticket(
    ...
    # BUG: appends the ticket-specific text into the "static" system prompt,
    # so it's no longer identical call to call -- defeats cache-friendliness.
    system_prompt = build_triage_prompt() + f"\n\nThe current ticket says: {ticket_message}"
