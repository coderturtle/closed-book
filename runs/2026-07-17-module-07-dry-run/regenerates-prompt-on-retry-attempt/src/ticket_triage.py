...
def classify_ticket(
    ...
    for _ in range(max_retries + 1):
        # BUG: mirrors Module 02's own retry pattern (embed the error into
        # the system prompt itself) -- fine there, but here it means the
        # "static" system prompt changes on every retry, defeating the
        # cache-friendliness this module's design actually requires.
        ...
        raw = model_client(current_system_prompt, ticket_message)
        error = _validate(raw)
        if error is None:
