...
def _validate(raw: dict) -> Optional[str]:
    ...
    if raw["category"] == "other" and not raw.get("detail"):
...
def classify_ticket(
    ...
    # BUG: no retry loop at all -- a single attempt, ignoring max_retries.
    raw = model_client(system_prompt, ticket_message)
    error = _validate(raw)
    if error is None:
        return TicketClassification(
            category=raw["category"], confidence=raw["confidence"], detail=raw.get("detail")
        )
    raise TriageFailed(f"could not classify ticket: {error}")
