...
def run_full_support_session(
    ...
    # `extraction` (and its `confidence` field, describing how confident the
    # *extraction step itself* was in parsing the message) is surfaced in
    # every returned result below for a human's audit trail -- it is never
    # used to drive control flow here. should_escalate (Module 05) is the
    # only escalation authority; its own signature has no confidence
    # parameter by design, and this integration must not defeat that
    # guarantee by adding a parallel check outside it.
    ...
    for i in range(max_iterations):
        ...
        if stop_reason == "end_turn":
            return {"stop_reason": "end_turn", "text": response.get("text", ""), "facts": facts, "extraction": extraction}
        ...
        if reason is not None:
            return {"escalated": True, "reason": reason, "facts": facts, "extraction": extraction}

    return {"stop_reason": "max_iterations", "text": "", "facts": facts, "extraction": extraction}
