...
class SessionState:
    ...
    must not be treated as having verified customer B.
    """
    ...
    for _ in range(max_iterations):
        ...
        if not result.get("error"):
            ...
            if tool_name == "get_customer":
                # BUG: trusts the identifier the caller *asked* to look up,
                # not the customer_id the backend actually verified in the
                # result -- these are not guaranteed to be the same value.
                session.verified_customer_id = tool_args.get("identifier")
