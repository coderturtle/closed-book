...
def update_case_facts(case_facts: CaseFacts, tool_name: str, tool_args: dict, tool_result: dict) -> CaseFacts:
    ...
    for field_name, result_key in field_map.items():
        if tool_name == "process_refund":
            # BUG: trusts what was requested, not what the backend actually
            # did -- a partial/capped refund would be recorded as the full
            # requested amount.
            if "amount_cents" not in tool_args:
                continue
            new_value = tool_args["amount_cents"]
        elif tool_name == "lookup_order":
            # BUG: same class of mistake -- order_id happens to share a key
            # name between tool_args and tool_result, making it easy to
            # accidentally read the request instead of the result.
            if "order_id" not in tool_args:
                continue
            new_value = tool_args["order_id"]
        else:
