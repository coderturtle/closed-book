...
def should_escalate(case_facts: CaseFacts, iterations_used: int, max_iterations: int) -> Optional[str]:
    ...
    BUG: uses iteration count alone as a proxy for "the session is going
    poorly," ignoring case_facts.conflicts and case_facts.error_count
    entirely -- exactly the "unreliable proxy" anti-pattern this module's
    own docstring warns against, just applied to an internal signal
    instead of the model's self-reported confidence.
    """
